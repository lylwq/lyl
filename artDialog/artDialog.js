	...
	var load, $iframe, iwin,
		...
		data = {
			...
			closeFn: function(here){
				...
				// IE注销iframe仍然留在内存中，置换src是最容易解决的方法
				$iframe[0].src = 'javascript:false';
				opt.closeFn && opt.closeFn.call(api, here);
			}
		};
		...
		data = {
			...
			initFn: function(here){
				...
				_load(url, function(content){
					...
					if (tmpl) content = $.tmpl(tmpl, eval('(' + content + ')'));
					// jQuery的html()方法可以执行script标签内容
					jq ? jq(api.ui.content[0]).html(content) : api.ui.content[0].innerHTML = content;
					api.data.left === 'center' && api.data.top === 'center' && api.position('center', 'center');
