...
query.fn = query.prototype = { 
	init: function(selector){
		console.log(this);
		var ret=this||[];
		if(selector) push.call(ret,selector);
		return ret;
		/*if(selector) selector.prototype = this;
	...
	parent : function(){
		var res=[];
		for(var i=0,dl=this.length;i<dl;i+=1){
	...
	removeClass : function(c_n){
		var i,dl=this.length;
		if(!c_n){
			for(i=0;i<dl;i+=1){this[i].className=''}
		}else{
			for(i=0;i<dl;i+=1){
				var oc=this[i].className;
				this[i].className=oc.replace(c_n,"")
			}
		}
		return this;
	},
	hasClass : function(c_n){
		var i,dl=this.length;
		for(i=0;i<dl;i+=1){
			if(this[i].className.indexOf(c_n)>-1)return true;
		}
		return false;		
	},
	filterByAttr : function(attr,at_val){
	    ...
	    for(var i=0,len=this.length;i<len;i+=1){
		    if(at_val&&this[i].getAttribute(attr)&&this[i].getAttribute(attr).toString().indexOf(at_val)>-1){
			    result.push(this[i]);
			    continue;
		    }
		    if(arguments.length===1&&!at_val&&this[i].getAttribute(attr)){
			    result.push(this[i]);
		    }
	    }
   ...
   selectAll : function(state){
   		for(var i=0,ln=this.length;i<ln;i+=1){
   			td[i].checked=state;
   		}
   }
};
