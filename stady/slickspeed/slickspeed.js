...
window.onload =function(){
	document.body.innerHTML=(new jCT(document.body.innerHTML)).Build().GetView();
	var frameworks = {};
	forEach(document.getElementsByTagName('iframe'), function(iframe){
		frameworks[iframe.name] = {
			'window': window.frames[iframe.name],
			'selectors': []
		};
	});
	...
	var testRunner = function(){
		...
		if (!test || !test.execute.test) return;
		var results = test.execute.test(test.selector);
